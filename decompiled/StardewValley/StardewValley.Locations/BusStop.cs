...
public class BusStop : GameLocation
{
	...
	{
		switch (getTileIndexAt(tileLocation, "Buildings", "outdoors"))
		{
		case 958:
		case 1080:
		case 1081:
		...
		case 1057:
			if (Game1.MasterPlayer.mailReceived.Contains("ccVault"))
			{
				if (!Game1.player.isRidingHorse() || Game1.player.mount == null)
				{
					...
					{
						createQuestionDialogueWithCustomWidth(Game1.content.LoadString("Strings\\Locations:BusStop_BuyTicketToDesert", numberWithCommas), createYesNoResponses(), "Bus");
					}
					else
					{
						createQuestionDialogue(Game1.content.LoadString("Strings\\Locations:BusStop_BuyTicketToDesert", numberWithCommas), createYesNoResponses(), "Bus");
					}
					break;
				}
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		Object objectAtTile = getObjectAtTile(25, 10);
	...
	public override bool answerDialogue(Response answer)
	{
		if (lastQuestionKey != null && afterQuestion == null && ArgUtility.SplitBySpaceAndGet(lastQuestionKey, 0) + "_" + answer.responseKey == "Bus_Yes")
		{
			...
			if (!Game1.netWorldState.Value.canDriveYourselfToday.Value && (!characters.Contains(characterFromName) || characterFromName.TilePoint.X != 21 || characterFromName.TilePoint.Y != 10))
			{
	...
	protected override void resetLocalState()
	{
		...
		if (locationFromName != null && locationFromName.GetLocationContext() != GetLocationContext())
		{
			flag = true;
		}
		if (Game1.player.TilePoint.Y > 16 || Game1.eventUp || Game1.player.TilePoint.X <= 10 || Game1.player.isRidingHorse() || !flag)
		{
			drivingOff = false;
			drivingBack = false;
