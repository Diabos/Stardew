...
public class Railroad : GameLocation
{
	...
	static Railroad()
	{
		...
		DailyTrainChance = (1.0 - Math.Sqrt(1.0 - 4.0 * num)) * 0.5;
	}
	...
	internal void ResetTrainForNewDay()
	{
		...
		Random random2 = Utility.CreateRandom(Game1.stats.DaysPlayed + 1, Game1.uniqueIDForThisGame / 2);
		bool num = random.NextDouble() < DailyTrainChance;
		bool flag = random2.NextDouble() < DailyTrainChance;
		if (num && !flag && Game1.isLocationAccessible("Railroad"))
		{
			trainTime = 900;
	...
	public void PlayTrainApproach()
	{
		...
		if (flag.HasValue && flag == true && !Game1.isFestival() && Game1.currentLocation.InValleyContext())
		{
			...
			Game1.playSound("distantTrain", out var cue);
