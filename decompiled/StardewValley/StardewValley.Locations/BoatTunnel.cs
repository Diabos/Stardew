...
public class BoatTunnel : GameLocation
{
	...
	public override bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
	{
		if (doesTileHaveProperty(tileLocation.X, tileLocation.Y, "Action", "Buildings") == "BoatTicket")
		{
			...
			else if (Game1.MasterPlayer.hasOrWillReceiveMail("willyBoatFixed"))
			{
				...
				{
					string numberWithCommas = Utility.getNumberWithCommas(TicketPrice);
					if (Game1.content.GetCurrentLanguage() == LocalizedContentManager.LanguageCode.es)
					{
						createQuestionDialogueWithCustomWidth(Game1.content.LoadString("Strings\\Locations:BuyTicket", numberWithCommas), createYesNoResponses(), "Boat");
					}
					else
					{
						createQuestionDialogue(Game1.content.LoadString("Strings\\Locations:BuyTicket", numberWithCommas), createYesNoResponses(), "Boat");
					}
				}
			}
			return true;
	...
	public override bool answerDialogue(Response answer)
	{
		...
		{
			string text = ArgUtility.SplitBySpaceAndGet(lastQuestionKey, 0) + "_" + answer.responseKey;
	...
	public virtual void StartDeparture()
	{
		...
		_boatEvent = new Event(stringBuilder.ToString(), null, "-78765", Game1.player)
		{
		...
		_farmerActor.faceDirection(facingDirection);
	...
	public override bool RunLocationSpecificEventCommand(Event current_event, string command_string, bool first_run, params string[] args)
	{
		...
		case "path_player":
		{
			int num = ArgUtility.GetInt(args, 0);
			int num2 = ArgUtility.GetInt(args, 1);
			int num3 = ArgUtility.GetInt(args, 2, 2);
			if (first_run)
			{
				_playerPathing = true;
				Game1.player.controller = new PathFindController(Game1.player, this, new Point(num, num2), num3, OnReachedBoatDeck)
				{
			...
			if (forceWarpTimer > 0)
			{
				...
				if (forceWarpTimer <= 0)
				{
					forceWarpTimer = 0;
					Game1.player.controller = null;
					Game1.player.setTileLocation(new Vector2(num, num2));
					Game1.player.faceDirection(num3);
