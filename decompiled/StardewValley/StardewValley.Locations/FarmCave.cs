...
public class FarmCave : GameLocation
{
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		if (Game1.MasterPlayer.caveChoice.Value == 1)
		{
			while (Game1.random.NextDouble() < 0.66)
			{
				...
				Vector2 vector = new Vector2(Game1.random.Next(1, map.Layers[0].LayerWidth - 1), Game1.random.Next(1, map.Layers[0].LayerHeight - 4));
	...
	public virtual void UpdateReadyFlag()
	{
		...
		{
			if (value2.isSpawnedObject.Value)
			{
				value = true;
				break;
			}
			if (value2.bigCraftable.Value && value2.heldObject.Value != null && value2.MinutesUntilReady <= 0 && value2.QualifiedItemId == "(BC)128")
			{
				value = true;
				break;
			}
		}
		Game1.getFarm().farmCaveReady.Value = value;
	}
	...
	public void setUpMushroomHouse()
	{
		...
		foreach (int num in array)
		{
			...
			{
				Object obj = ItemRegistry.Create<Object>("(BC)128");
				obj.fragility.Value = 2;
				setObject(new Vector2(num2, num), obj);
			}
		}
