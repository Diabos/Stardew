...
public class FishingQuest : Quest
{
	...
	public void loadQuestInfo()
	{
		...
		if (random.NextBool())
		{
			...
			bool flag = ItemId.Value == "(O)149";
			numberToFish.Value = (int)Math.Ceiling(90.0 / (double)Math.Max(1, GetGoldRewardPerItem(item))) + Game1.player.FishingLevel / 5;
			reward.Value = numberToFish.Value * GetGoldRewardPerItem(item);
			target.Value = "Demetrius";
			parts.Clear();
			parts.Add(new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13228", item, numberToFish.Value));
			dialogueparts.Clear();
			dialogueparts.Add(new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13231", item, random.Choose(new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13233"), new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13234"), new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13235"), new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13236", item))));
			objective.Value = (flag ? new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13243", 0, numberToFish.Value) : new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13244", 0, numberToFish.Value, item));
		}
		else
		{
			switch (Game1.season)
			...
			target.Value = "Willy";
			Item item2 = ItemRegistry.Create(ItemId.Value);
			bool flag2 = ItemId.Value == "(O)151";
			numberToFish.Value = (int)Math.Ceiling(90.0 / (double)Math.Max(1, GetGoldRewardPerItem(item2))) + Game1.player.FishingLevel / 5;
			reward.Value = numberToFish.Value * GetGoldRewardPerItem(item2);
			parts.Clear();
			parts.Add(flag2 ? new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13248", reward.Value, numberToFish.Value, new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13253")) : new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13248", reward.Value, numberToFish.Value, item2));
			dialogueparts.Clear();
			dialogueparts.Add(new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13256", item2));
			...
			objective.Value = (flag2 ? new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13255", 0, numberToFish.Value) : new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13244", 0, numberToFish.Value, item2));
		}
	...
	public override void reloadObjective()
	{
		...
		{
			objective.Value = (flag ? new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13243", numberFished.Value, numberToFish.Value) : (flag2 ? new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13255", numberFished.Value, numberToFish.Value) : new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13244", numberFished.Value, numberToFish.Value, ItemRegistry.Create(ItemId.Value))));
		}
	...
	public override bool OnFishCaught(string fishId, int numberCaught, int size, bool probe = false)
	{
		...
		loadQuestInfo();
		if (fishId == ItemId.Value && numberFished.Value < numberToFish.Value)
		{
			if (!probe)
			{
	...
	public override bool OnNpcSocialized(NPC npc, bool probe = false)
	{
		...
		if (numberFished.Value >= numberToFish.Value && target.Value != null && npc.Name == target.Value && npc.IsVillager && !completed.Value)
		{
			if (!probe)
			{
