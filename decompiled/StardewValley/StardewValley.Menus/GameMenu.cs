...
public class GameMenu : IClickableMenu
{
		...
		: base(Game1.uiViewport.Width / 2 - (800 + IClickableMenu.borderWidth * 2) / 2, Game1.uiViewport.Height / 2 - (600 + IClickableMenu.borderWidth * 2) / 2, 800 + IClickableMenu.borderWidth * 2, 600 + IClickableMenu.borderWidth * 2, showUpperRightCloseButton: true)
	{
		...
		int num = ((LocalizedContentManager.CurrentLanguageCode == LocalizedContentManager.LanguageCode.ru) ? 96 : ((LocalizedContentManager.CurrentLanguageCode == LocalizedContentManager.LanguageCode.tr || LocalizedContentManager.CurrentLanguageCode == LocalizedContentManager.LanguageCode.fr) ? 192 : 0));
		pages.Add(new OptionsPage(xPositionOnScreen, yPositionOnScreen, width + num, height));
	...
	public override void receiveGamePadButton(Buttons button)
	{
		...
		case Buttons.LeftTrigger:
			...
			break;
		default:
			pages[currentTab].receiveGamePadButton(button);
			break;
		}
	}
	...
	{
		if (!TabTranslationKeys.TryGetValue(index, out var value))
		{
			return "";
		}
		return Game1.content.LoadString(value);
	}
		...
		if (!invisible)
		{
			...
			foreach (ClickableComponent tab in tabs)
			{
				...
				case "coop":
					...
					break;
				case "powers":
					b.Draw(Game1.mouseCursors_1_6, new Vector2(tab.bounds.X, tab.bounds.Y + ((currentTab == getTabNumberFromName(tab.name)) ? 8 : 0)), new Rectangle(216, 494, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.0001f);
					break;
				case "animals":
					b.Draw(Game1.mouseCursors_1_6, new Vector2(tab.bounds.X, tab.bounds.Y + ((currentTab == getTabNumberFromName(tab.name)) ? 8 : 0)), new Rectangle(257, 246, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.0001f);
					break;
				}
				if (num != -1)
				{
					b.Draw(Game1.mouseCursors, new Vector2(tab.bounds.X, tab.bounds.Y + ((currentTab == getTabNumberFromName(tab.name)) ? 8 : 0)), new Rectangle(num * 16, 368, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.0001f);
				}
				if (tab.name.Equals("skills"))
				{
					Game1.player.FarmerRenderer.drawMiniPortrat(b, new Vector2(tab.bounds.X + 8, tab.bounds.Y + 12 + ((currentTab == getTabNumberFromName(tab.name)) ? 8 : 0)), 0.00011f, 3f, 2, Game1.player);
				}
			}
			b.End();
