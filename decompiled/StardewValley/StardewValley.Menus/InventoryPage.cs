...
public class InventoryPage : IClickableMenu
{
		...
		: base(x, y, width, height)
	{
		...
		int rightNeighborID = (num ? 120 : 105);
		...
		if (num)
		{
			...
			{
				ClickableComponent clickableComponent = new ClickableComponent(new Rectangle(xPositionOnScreen + 48 + 280, yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 4 + (4 + i) * 64 - 12, 64, 64), "Trinket")
				{
					myID = 120 + i,
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		foreach (ClickableComponent equipmentIcon in equipmentIcons)
		{
			...
			Item item = Utility.PerformSpecialItemPlaceReplacement(Game1.player.CursorSlotItem);
			bool flag = item == null;
			switch (equipmentIcon.name)
			{
			case "Hat":
				if (item == null || item is Hat)
				{
					Item heldItem2 = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Hat)item, Game1.player.hat));
			...
			case "Right Ring":
				if (item == null || item is Ring)
				{
					...
					Item heldItem6 = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Ring)item, slot));
				...
				break;
			case "Boots":
				if (item == null || item is Boots)
				{
					Item heldItem3 = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Boots)item, Game1.player.boots));
					setHeldItem(heldItem3);
			...
			case "Shirt":
			{
				if (item != null)
				{
					Clothing obj2 = item as Clothing;
					if (obj2 == null || obj2.clothesType.Value != Clothing.ClothesType.SHIRT)
					{
						break;
					}
				}
				Item heldItem5 = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Clothing)item, Game1.player.shirtItem));
				setHeldItem(heldItem5);
			...
			case "Pants":
			{
				if (item != null)
				{
					Clothing obj = item as Clothing;
					if (obj == null || obj.clothesType.Value != Clothing.ClothesType.PANTS)
					{
						break;
					}
				}
				Item heldItem4 = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Clothing)item, Game1.player.pantsItem));
			...
			case "Trinket":
				if (Game1.player.stats.Get("trinketSlots") != 0 && checkHeldItem((Item item2) => item2 == null || item2 is Trinket))
				{
					int num = equipmentIcon.myID - 120;
					Trinket value = (Trinket)takeHeldItem();
					Trinket heldItem = null;
					if (Game1.player.trinketItems.Count > num)
					{
						heldItem = Game1.player.trinketItems[num];
					}
					heldItem = (Trinket)Utility.PerformSpecialItemGrabReplacement(heldItem);
					setHeldItem(heldItem);
					while (Game1.player.trinketItems.Count <= num)
					{
						Game1.player.trinketItems.Add(null);
					}
					Game1.player.trinketItems[num] = value;
					if (Game1.player.trinketItems[num] != null)
					{
			...
			if (!flag || !checkHeldItem() || !Game1.oldKBState.IsKeyDown(Keys.LeftShift))
			{
				continue;
			}
			int i;
			...
			{
				if (Game1.player.Items[i] == null || checkHeldItem((Item other) => Game1.player.Items[i].canStackWith(other)))
				{
					if (Game1.player.CurrentToolIndex == i && checkHeldItem())
					{
						Game1.player.CursorSlotItem.actionWhenBeingHeld(Game1.player);
					}
					setHeldItem(Utility.addItemToInventory(takeHeldItem(), i, inventory.actualInventory));
					if (Game1.player.CurrentToolIndex == i && checkHeldItem())
					{
		...
		if (checkHeldItem((Item item2) => item2?.QualifiedItemId == "(O)434"))
		{
		...
		{
			if (checkHeldItem((Item item2) => item2 is Ring))
			{
				if (Game1.player.leftRing.Value == null)
				{
			...
			else if (checkHeldItem((Item item2) => item2 is Hat))
			{
				if (Game1.player.hat.Value == null)
				{
			...
			else if (checkHeldItem((Item item2) => item2 is Boots))
			{
				if (Game1.player.boots.Value == null)
				{
			...
			else if (checkHeldItem((Item item2) => item2 is Clothing clothing && clothing.clothesType.Value == Clothing.ClothesType.SHIRT))
			{
				if (Game1.player.shirtItem.Value == null)
				{
			...
			else if (checkHeldItem((Item item2) => item2 is Clothing clothing && clothing.clothesType.Value == Clothing.ClothesType.PANTS))
			{
				if (Game1.player.pantsItem.Value == null)
				{
			...
			else if (checkHeldItem((Item item2) => item2 is Trinket) && Game1.player.stats.Get("trinketSlots") != 0)
			{
				bool flag2 = false;
				for (int num2 = 0; num2 < Game1.player.trinketItems.Count; num2++)
				{
					if (Game1.player.trinketItems[num2] == null)
					{
						Game1.player.trinketItems[num2] = takeHeldItem() as Trinket;
						flag2 = true;
						break;
					}
				}
			...
			if (inventory.getInventoryPositionOfClick(x, y) >= 12)
			{
				...
				for (i2 = 0; i2 < 12; i2++)
				{
					if (Game1.player.Items[i2] == null || checkHeldItem((Item other) => Game1.player.Items[i2].canStackWith(other)))
					{
						if (Game1.player.CurrentToolIndex == i2 && checkHeldItem())
						{
							Game1.player.CursorSlotItem.actionWhenBeingHeld(Game1.player);
						}
						setHeldItem(Utility.addItemToInventory(takeHeldItem(), i2, inventory.actualInventory));
						if (checkHeldItem())
						{
							Game1.player.CursorSlotItem.actionWhenStopBeingHeld(Game1.player);
						}
			...
			{
				int i3;
				for (i3 = 12; i3 < Game1.player.Items.Count; i3++)
				{
					if (Game1.player.Items[i3] == null || checkHeldItem((Item other) => Game1.player.Items[i3].canStackWith(other)))
					{
						if (Game1.player.CurrentToolIndex == i3 && checkHeldItem())
						{
							Game1.player.CursorSlotItem.actionWhenBeingHeld(Game1.player);
						}
						setHeldItem(Utility.addItemToInventory(takeHeldItem(), i3, inventory.actualInventory));
						if (checkHeldItem())
						{
							Game1.player.CursorSlotItem.actionWhenStopBeingHeld(Game1.player);
						}
		...
		if (trashCan.containsPoint(x, y) && checkHeldItem((Item item2) => item2?.canBeTrashed() ?? false))
		{
			Utility.trashItem(takeHeldItem());
		...
		else if (!isWithinBounds(x, y) && checkHeldItem((Item item2) => item2?.canBeTrashed() ?? false))
		{
	...
	public override void draw(SpriteBatch b)
	{
		...
		foreach (ClickableComponent equipmentIcon in equipmentIcons)
		{
			...
			case "Hat":
				if (Game1.player.hat.Value != null)
				{
					...
					Game1.player.hat.Value.drawInMenu(b, new Vector2(equipmentIcon.bounds.X, equipmentIcon.bounds.Y), equipmentIcon.scale, 1f, 0.866f, StackDrawType.Hide);
				}
				else
				{
			...
			case "Right Ring":
				...
				{
					b.Draw(Game1.menuTexture, equipmentIcon.bounds, Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 10), Color.White);
					Game1.player.rightRing.Value.drawInMenu(b, new Vector2(equipmentIcon.bounds.X, equipmentIcon.bounds.Y), equipmentIcon.scale);
				}
				else
				{
			...
			case "Left Ring":
				...
				{
					b.Draw(Game1.menuTexture, equipmentIcon.bounds, Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 10), Color.White);
					Game1.player.leftRing.Value.drawInMenu(b, new Vector2(equipmentIcon.bounds.X, equipmentIcon.bounds.Y), equipmentIcon.scale);
				}
				else
				{
					b.Draw(Game1.menuTexture, equipmentIcon.bounds, Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 41), Color.White);
			...
			case "Boots":
				...
				{
					b.Draw(Game1.menuTexture, equipmentIcon.bounds, Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 10), Color.White);
					Game1.player.boots.Value.drawInMenu(b, new Vector2(equipmentIcon.bounds.X, equipmentIcon.bounds.Y), equipmentIcon.scale);
				}
				else
				{
			...
			case "Shirt":
				...
				{
					b.Draw(Game1.menuTexture, equipmentIcon.bounds, Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 10), Color.White);
					Game1.player.shirtItem.Value.drawInMenu(b, new Vector2(equipmentIcon.bounds.X, equipmentIcon.bounds.Y), equipmentIcon.scale);
				}
				else
				{
			...
			case "Pants":
				...
				{
					b.Draw(Game1.menuTexture, equipmentIcon.bounds, Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 10), Color.White);
					Game1.player.pantsItem.Value.drawInMenu(b, new Vector2(equipmentIcon.bounds.X, equipmentIcon.bounds.Y), equipmentIcon.scale);
				}
				else
				{
			...
			{
				int num = equipmentIcon.myID - 120;
				if (Game1.player.trinketItems.Count > num && Game1.player.trinketItems[num] != null)
				{
					b.Draw(Game1.menuTexture, equipmentIcon.bounds, Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, 10), Color.White);
					Game1.player.trinketItems[num].drawInMenu(b, new Vector2(equipmentIcon.bounds.X, equipmentIcon.bounds.Y), equipmentIcon.scale);
				}
				else
				{
		...
		float num2 = 32f;
		string text = Game1.content.LoadString("Strings\\UI:Inventory_FarmName", Game1.player.farmName);
		Utility.drawTextWithShadow(b, text, Game1.dialogueFont, new Vector2((float)xPositionOnScreen + num2 + 512f + 32f - Game1.dialogueFont.MeasureString(text).X / 2f, yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 256 + 4), Game1.textColor);
		string text2 = Game1.content.LoadString("Strings\\UI:Inventory_CurrentFunds" + (Game1.player.useSeparateWallets ? "_Separate" : ""), Utility.getNumberWithCommas(Game1.player.Money));
		Utility.drawTextWithShadow(b, text2, Game1.dialogueFont, new Vector2((float)xPositionOnScreen + num2 + 512f + 32f - Game1.dialogueFont.MeasureString(text2).X / 2f, yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 320 + 4), Game1.textColor);
		string text3 = Game1.content.LoadString("Strings\\UI:Inventory_TotalEarnings" + (Game1.player.useSeparateWallets ? "_Separate" : ""), Utility.getNumberWithCommas((int)Game1.player.totalMoneyEarned));
		Utility.drawTextWithShadow(b, text3, Game1.dialogueFont, new Vector2((float)xPositionOnScreen + num2 + 512f + 32f - Game1.dialogueFont.MeasureString(text3).X / 2f, yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 384), Game1.textColor);
		Utility.drawTextWithShadow(b, Utility.getDateString(), Game1.dialogueFont, new Vector2((float)xPositionOnScreen + num2 + 512f + 32f - Game1.dialogueFont.MeasureString(Utility.getDateString()).X / 2f, yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 448), Game1.textColor * 0.8f);
