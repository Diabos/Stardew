...
public class OptionsPage : IClickableMenu
{
		...
		: base(x, y, width, height)
	{
		...
		bool flag = Game1.game1.IsMainInstance && Game1.game1.IsLocalCoopJoinable();
		if (Game1.multiplayerMode == 2 || flag)
		{
		...
		for (int num = 75; num <= 150; num += 5)
		{
			list.Add(num + "%");
		}
		options.Add(new OptionsPlusMinus(Game1.content.LoadString("Strings\\StringsFromCSFiles:OptionsPage_UIScale"), 39, list, list));
		list = new List<string>();
		for (int num2 = 75; num2 <= 200; num2 += 5)
		{
			list.Add(num2 + "%");
		}
		options.Add(new OptionsCheckbox(Game1.content.LoadString("Strings\\StringsFromCSFiles:OptionsPage.cs.11253"), 15));
		options.Add(new OptionsPlusMinus(Game1.content.LoadString("Strings\\StringsFromCSFiles:OptionsPage.cs.11254"), 18, list, list));
		...
		{
			OptionsButton optionsButton = new OptionsButton(Game1.content.LoadString("Strings\\UI:OptionsPage_ScreenshotHeader").Replace(":", ""), TakeScreenshot);
		...
		void TakeScreenshot()
		{
			...
			Game1.flashAlpha = 1f;
			e.greyedOut = true;
			string text = Game1.game1.takeMapScreenshot(null, null, OnDone);
			if (text != null)
			{
				Game1.addHUDMessage(new HUDMessage(text, 6));
			}
	...
	{
		if (oldPage is OptionsPage optionsPage)
		{
