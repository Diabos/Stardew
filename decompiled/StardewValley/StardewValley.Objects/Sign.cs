...
public class Sign : Object
{
	...
	public override bool checkForAction(Farmer who, bool justCheckingForActivity = false)
	{
		...
		if (currentItem != null)
		{
			...
			displayItem.Value = currentItem.getOne();
			...
			if (!(value is Hat))
			{
				if (!(value is Ring))
				{
					if (!(value is Furniture))
					{
						...
						{
							displayType.Value = ((!obj.bigCraftable.Value) ? 1 : 3);
						}
					}
					else
					{
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		if (displayItem.Value != null)
		{
			...
			{
			case 1:
				displayItem.Value.drawInMenu(spriteBatch, Game1.GlobalToLocal(Game1.viewport, new Vector2((float)(x * 64) + 1f, y * 64 - 64 + 21 + 8 - 2)), 0.75f, 0.45f, Math.Max(0f, (float)((y + 1) * 64 - 24) / 10000f) + (float)x * 1E-05f + 1E-05f, StackDrawType.Hide, Color.Black, drawShadow: false);
	...
	public override bool ForEachItem(ForEachItemDelegate handler, GetForEachItemPathDelegate getPath)
	{
		...
		{
			return ForEachItemHelper.ApplyToField(displayItem, handler, getPath);
		}
		return false;
	}
}
