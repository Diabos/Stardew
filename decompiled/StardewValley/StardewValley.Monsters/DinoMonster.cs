...
public class DinoMonster : Monster
{
	...
	public class BreathProjectile : INetObject<NetFields>
	{
		...
		public void Update(GameTime time, GameLocation location, DinoMonster parent)
		{
			...
			float num = Vector2.Distance(position.Value, startPosition.Value);
			if (num > 128f)
			{
				alpha = (256f - num) / 128f;
			}
		...
		public void Draw(SpriteBatch b)
		{
			if (active.Value)
			{
				...
				Rectangle sourceRect = GetSourceRect();
				Vector2 value = position.Value;
				b.Draw(projectileSheet, Game1.GlobalToLocal(Game1.viewport, value + new Vector2(32f, 32f)), sourceRect, Color.White * alpha, rotation, new Vector2(8f, 8f), scale, SpriteEffects.None, (value.Y + 96f) / 10000f);
			}
		}
	}
	...
	public override void draw(SpriteBatch b)
	{
		if (base.Health > 0 && !base.IsInvisible && Utility.isOnScreen(base.Position, 128))
		{
			...
			b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(56f, 16 + yJumpOffset), Sprite.SourceRect, Color.White, rotation, new Vector2(16f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)y / 10000f)));
			if (isGlowing)
			{
				b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(56f, 16 + yJumpOffset), Sprite.SourceRect, glowingColor * glowingTransparency, 0f, new Vector2(16f, 16f), 4f * Math.Max(0.2f, scale.Value), flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)y / 10000f + 0.001f)));
			}
		}
	...
	public override Rectangle GetBoundingBox()
	{
		...
		return new Rectangle((int)vector.X + 8, (int)vector.Y, Sprite.SpriteWidth * 4 * 3 / 4, 64);
	}
	...
	public override List<Item> getExtraDropItems()
	{
		...
		{
			list.Add(ItemRegistry.Create("(O)107"));
		}
		else
		{
			...
			list.Add(Game1.random.ChooseFrom(options));
		}
		return list;
	}
	...
	protected override void sharedDeathAnimation()
	{
		...
		for (int i = 0; i < 16; i++)
		{
			Game1.createRadialDebris(base.currentLocation, Sprite.textureName.Value, new Rectangle(64, 128, 16, 16), 16, (int)Utility.Lerp(boundingBox.Left, boundingBox.Right, (float)Game1.random.NextDouble()), (int)Utility.Lerp(boundingBox.Bottom, boundingBox.Top, (float)Game1.random.NextDouble()), 1, base.TilePoint.Y, Color.White, 4f);
		}
	}
	...
	public override void behaviorAtGameTick(GameTime time)
	{
		...
		else
		{
			...
			if (wanderState)
			{
				...
				tryToMoveInDirection(facingDirection.Value, isFarmer: false, base.DamageToFarmer, isGlider.Value);
			}
		}
		...
		else
		{
			...
			if (nextFireTime > 0)
			{
				...
				if (nextFireTime <= 0)
				{
					...
					Vector2 vector = new Vector2((float)standingPixel.X - 32f, (float)standingPixel.Y - 32f);
					...
					Vector2 value = new Vector2((float)Math.Cos((double)num * Math.PI / 180.0), 0f - (float)Math.Sin((double)num * Math.PI / 180.0));
					value *= 10f;
					BreathProjectile breathProjectile = projectiles[lastProjectileSlot];
					breathProjectile.active.Value = true;
					NetVector2 netVector = breathProjectile.position;
					Vector2 value2 = (breathProjectile.startPosition.Value = vector);
					netVector.Value = value2;
					breathProjectile.velocity.Value = value;
	...
	protected override void updateAnimation(GameTime time)
	{
		...
		if (attackState.Value == 1)
		{
			if (firing.Value)
			{
				Sprite.CurrentFrame = 16 + num;
			}
			else
			{
				Sprite.CurrentFrame = 17 + num;
			}
			return;
		}
