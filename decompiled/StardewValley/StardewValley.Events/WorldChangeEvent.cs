...
public class WorldChangeEvent : BaseFarmEvent
{
	...
	private void obliterateJojaMartDoor()
	{
		...
		town.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(372, 1956, 10, 10), new Vector2(96f, 50f) * 64f + new Vector2(Game1.random.Next(-32, 64), 0f), flipped: false, 0f, Color.Gray)
		{
			...
			interval = 99999f,
			layerDepth = 0.9f,
			lightId = GenerateLightSourceId() + "_obliterateJojaMartDoor",
			lightRadius = 4f,
	...
	public override bool setUp()
	{
		...
		case 1:
			...
			foreach (Building building in location.buildings)
			{
				...
				{
					targetTile = new Point(building.tileX.Value + 3, building.tileY.Value + 3);
					break;
				}
			}
			break;
	...
	public void resetForPlayerEntry(Point targetTile)
	{
		...
		string text = $"{"WorldChangeEvent"}_{whichEvent.Value}";
		switch (whichEvent.Value)
		{
		case 15:
		{
			...
			int num2 = 130 - Game1.netWorldState.Value.GoldenWalnutsFound;
			int num3 = 1 + num2 / 10;
			for (int k = 0; k < num3; k++)
			{
				location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(184, 104, 14, 15), new Vector2(39 + k % 3, 34.1f + (float)(k / 3) * 0.5f) * 64f, flipped: false, 0f, Color.White)
				{
					animationLength = 1,
					interval = 700f,
					totalNumberOfLoops = 999,
			...
			for (int l = 0; l < 20; l++)
			{
				Vector2 position3 = Utility.getRandomPositionInThisRectangle(new Rectangle(20, 1, 40, 2), Game1.random) * 64f;
				float num4 = ((!(position3.X > (float)(location.Map.DisplayWidth / 2))) ? 1 : (-1));
				TemporaryAnimatedSprite temporaryAnimatedSprite3 = new TemporaryAnimatedSprite("LooseSprites\\parrots", new Rectangle(48 + Game1.random.Next(2) * 72, 96, 24, 24), position3, flipped: false, 0f, Color.White)
				{
					motion = new Vector2(num4 * 3f, 6f + (float)Game1.random.NextDouble()),
					acceleration = new Vector2(0f, -0.01f),
					id = 778,
					scale = 4f,
					yStopCoordinate = (int)position3.Y + Game1.random.Next(19, 27) * 64 + l * 64 / 2,
				...
				DelayedAction.playSoundAfterDelay("parrot_flap", 500 + l * 250);
				DelayedAction.playSoundAfterDelay("parrot_flap", 5500 + l * 250);
				temporaryAnimatedSprite3.reachedStopCoordinateSprite = GoldenParrotBounce;
		...
		case 12:
		{
			...
			Vector2 vector = new Vector2(96f, 50f) * 64f;
			Vector2 position2 = vector + new Vector2(-sourceRect.Width * 4 / 2, -sourceRect.Height * 4);
			while (position2.Y > (float)(-sourceRect.Height * 4))
			{
				location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", sourceRect, 9999f, 1, 999, position2, flicker: false, Game1.random.NextBool(), (vector.Y + 32f) / 10000f + 0.001f, 0.025f, Color.White, 4f, 0f, 0f, 0f)
				{
					lightId = text,
					lightRadius = 2f,
				...
				position2.Y -= sourceRect.Height * 4;
			}
		...
		case 10:
			...
			soundInterval = 560;
			Game1.currentLightSources.Add(new LightSource(text, 4, new Vector2(targetTile.X, targetTile.Y) * 64f, 4f, LightSource.LightContext.None, 0L));
			sound = "axchop";
			break;
		case 11:
			...
			Game1.player.activeDialogueEvents.TryAdd("movieTheater", 3);
			sound = "junimoMeep1";
			Game1.currentLightSources.Add(new LightSource(text, 4, new Vector2(targetTile.X, targetTile.Y) * 64f, 4f, Color.DarkGoldenrod, LightSource.LightContext.None, 0L));
		...
		case 0:
		{
			...
			Game1.currentLightSources.Add(new LightSource(text, 4, new Vector2(targetTile.X, targetTile.Y) * 64f, 4f, LightSource.LightContext.None, 0L));
			sound = "axchop";
			float num6 = (float)((targetTile.Y + 3) * 64) / 10000f;
			TemporaryAnimatedSprite hole3 = new TemporaryAnimatedSprite("Buildings\\Greenhouse", new Rectangle(25, 133, 31, 19), 99999f, 1, 999, new Vector2(targetTile.X * 64, (targetTile.Y - 1) * 64 - 64) + new Vector2(-23f, 53f) * 4f, flicker: false, flipped: false)
			{
				scale = 4f,
				layerDepth = num6 + 0.0008f
			};
			location.temporarySprites.Add(hole3);
			TemporaryAnimatedSprite raccoon3 = new TemporaryAnimatedSprite("Characters\\raccoon", new Rectangle(0, 32, 32, 32), 99999f, 1, 999, new Vector2(targetTile.X * 64, (targetTile.Y - 1) * 64 - 64) + new Vector2(-20f, 40f) * 4f, flicker: false, flipped: false)
			{
				scale = 4f,
				shakeIntensity = 1f,
				layerDepth = num6 + 0.0004f,
			...
			raccoon3.reachedStopCoordinate = delegate
			{
				hole3.layerDepth = 0f;
				raccoon3.motion.X = -1f;
				raccoon3.yStopCoordinate = targetTile.Y * 64 + 72;
				raccoon3.reachedStopCoordinate = delegate
				{
					raccoon3.motion = new Vector2(0f, 4f);
					raccoon3.acceleration = Vector2.Zero;
					raccoon3.sourceRect = new Rectangle(0, 0, 32, 32);
					raccoon3.animationLength = 8;
					raccoon3.interval = 80f;
					raccoon3.sourceRectStartingPos = Vector2.Zero;
					raccoon3.yStopCoordinate = targetTile.Y * 64 + 160;
					raccoon3.reachedStopCoordinate = delegate
					{
						raccoon3.layerDepth = -1f;
						raccoon3.motion = new Vector2(0f, 4f);
						raccoon3.layerDepthOffset = 0.0128f;
					};
				};
			};
			location.temporarySprites.Add(raccoon3);
			break;
		}
		case 1:
		{
			...
			soundInterval = 800;
			float num5 = (float)((targetTile.Y + 3) * 64) / 10000f;
		...
		case 9:
			...
			sound = "junimoMeep1";
			Game1.currentLightSources.Add(new LightSource(text + "_1", 4, new Vector2(targetTile.X, targetTile.Y) * 64f, 1f, LightSource.LightContext.None, 0L));
			Game1.currentLightSources.Add(new LightSource(text + "_2", 4, new Vector2(targetTile.X, targetTile.Y) * 64f, 1f, Color.DarkCyan, LightSource.LightContext.None, 0L));
			Game1.currentLightSources.Add(new LightSource(text + "_3", 4, new Vector2(targetTile.X, targetTile.Y) * 64f, 4f, Color.DarkGoldenrod, LightSource.LightContext.None, 0L));
			soundInterval = 1000;
			break;
		case 14:
		{
			...
			for (int i = 0; i < 16; i++)
			{
				...
				TemporaryAnimatedSprite temporaryAnimatedSprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Rectangle(49 + 16 * Game1.random.Next(3), 229, 16, 6), Utility.getRandomPositionInThisRectangle(r, Game1.random) * 64f, Game1.random.NextBool(), 0f, Color.White)
				{
				...
				temporaryAnimatedSprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Rectangle(49 + 16 * Game1.random.Next(3), 229, 16, 6), Utility.getRandomPositionInThisRectangle(r, Game1.random) * 64f, Game1.random.NextBool(), 0f, Color.White)
				{
					motion = new Vector2(Game1.random.Next(-2, 3), -16f),
					acceleration = new Vector2(0f, 0.5f),
					rotationChange = (float)Game1.random.Next(-4, 5) * 0.05f,
			...
			for (int j = 0; j < 20; j++)
			{
				Vector2 position = new Vector2(Utility.RandomFloat(13f, 19f), 0f) * 64f;
				float num = 1024f - position.X;
				TemporaryAnimatedSprite temporaryAnimatedSprite2 = new TemporaryAnimatedSprite("LooseSprites\\parrots", new Rectangle(48 + Game1.random.Next(2) * 72, Game1.random.Next(2) * 48, 24, 24), position, flipped: false, 0f, Color.White)
				{
					motion = new Vector2(num * 0.01f, 10f),
				...
				DelayedAction.playSoundAfterDelay("batFlap", 3500 + j * 250);
				temporaryAnimatedSprite2.reachedStopCoordinateSprite = ParrotBounce;
	...
	public virtual void FinishTreehouse()
	{
		...
		for (int i = 0; i < 20; i++)
		{
			...
			float num = 1024f - position.X;
			TemporaryAnimatedSprite item = new TemporaryAnimatedSprite("LooseSprites\\parrots", new Rectangle(192, Game1.random.Next(2) * 48, 24, 24), position, flipped: false, 0f, Color.White)
			{
				motion = new Vector2(num * -0.01f, Utility.RandomFloat(-2f, 0f)),
				acceleration = new Vector2(0f, -0.05f),
				id = 778,
				scale = 4f,
				totalNumberOfLoops = 99999,
			...
			location.TemporarySprites.Add(item);
		}
	}
	...
	public override void makeChangesToLocation()
	{
		...
		if (whichEvent.Value == 15 && Game1.IsMasterGame)
		{
