...
public class WitchEvent : BaseFarmEvent
{
	...
	public override bool setUp()
	{
		...
		foreach (Building building in f.buildings)
		{
			...
			AnimalHouse animalHouse = (AnimalHouse)building.GetIndoors();
		...
		if (targetBuilding == null)
		{
			...
			{
				if (building2.buildingType.Value == "Slime Hutch")
				{
		...
		Game1.currentLightSources.Add(new LightSource(lightSourceId, 4, witchPosition, 2f, Color.Black, LightSource.LightContext.None, 0L));
		...
		Game1.viewport.X = Math.Max(0, Math.Min(f.map.DisplayWidth - Game1.viewport.Width, targetBuilding.tileX.Value * 64 - Game1.viewport.Width / 2));
		Game1.viewport.Y = Math.Max(0, Math.Min(f.map.DisplayHeight - Game1.viewport.Height, (targetBuilding.tileY.Value - 3) * 64 - Game1.viewport.Height / 2));
		witchPosition = new Vector2(Game1.viewport.X + Game1.viewport.Width + 128, targetBuilding.tileY.Value * 64 - 64);
	...
	public override bool tickUpdate(GameTime time)
	{
		...
		Utility.repositionLightSource(lightSourceId, witchPosition + new Vector2(32f, 32f));
		if (animationLoopsDone < 1)
		{
			timerSinceFade += time.ElapsedGameTime.Milliseconds;
		}
		if (witchPosition.X > (float)(targetBuilding.tileX.Value * 64 + 96))
		{
	...
	public override void makeChangesToLocation()
	{
		...
		if (targetBuilding.buildingType.Value == "Slime Hutch")
		{
			foreach (NPC character in indoors.characters)
			{
				...
				{
					greenSlime.color.Value = new Color(40 + r.Next(10), 40 + r.Next(10), 40 + r.Next(10));
				}
			}
			return;
		}
		for (int i = 0; i < 200; i++)
		{
			Vector2 vector = new Vector2(r.Next(2, indoors.Map.Layers[0].LayerWidth - 2), r.Next(2, indoors.Map.Layers[0].LayerHeight - 2));
			if ((indoors.CanItemBePlacedHere(vector) || (indoors.terrainFeatures.TryGetValue(vector, out var value) && value is Flooring)) && !indoors.objects.ContainsKey(vector))
			{
				Object obj = ItemRegistry.Create<Object>(goldenWitch ? "(O)928" : "(O)305");
				obj.CanBeSetDown = false;
				obj.IsSpawnedObject = true;
				indoors.objects.Add(vector, obj);
				break;
			}
		}
	}
