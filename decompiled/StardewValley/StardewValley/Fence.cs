...
public class Fence : Object
{
	...
	public int getDrawSum()
	{
		...
		value.X += 1f;
		if (location.objects.TryGetValue(value, out var value2) && value2 is Fence fence && fence.countsForDrawing(base.ItemId))
		{
			num += 100;
		}
		value.X -= 2f;
		if (location.objects.TryGetValue(value, out var value3) && value3 is Fence fence2 && fence2.countsForDrawing(base.ItemId))
		{
			num += 10;
		}
		value.X += 1f;
		value.Y += 1f;
		if (location.objects.TryGetValue(value, out var value4) && value4 is Fence fence3 && fence3.countsForDrawing(base.ItemId))
		{
			num += 500;
		}
		value.Y -= 2f;
		if (location.objects.TryGetValue(value, out var value5) && value5 is Fence fence4 && fence4.countsForDrawing(base.ItemId))
		{
			num += 1000;
		}
		return num;
	...
	public override bool checkForAction(Farmer who, bool justCheckingForActivity = false)
	{
		...
		if (!justCheckingForActivity && who != null)
		{
			...
			if (location.objects.TryGetValue(new Vector2(tilePoint.X, tilePoint.Y - 1), out var value) && location.objects.TryGetValue(new Vector2(tilePoint.X, tilePoint.Y + 1), out var value2) && location.objects.TryGetValue(new Vector2(tilePoint.X + 1, tilePoint.Y), out var value3) && location.objects.TryGetValue(new Vector2(tilePoint.X - 1, tilePoint.Y), out var value4) && !value.isPassable() && !value2.isPassable() && !value4.isPassable() && !value3.isPassable())
			{
		...
		if (isGate.Value)
		{
			...
			if (isGate.Value)
			{
				toggleGate(who, gatePosition.Value == 0);
			}
			return true;
		}
		if (justCheckingForActivity)
		...
		foreach (Vector2 adjacentTileLocation in Utility.getAdjacentTileLocations(tileLocation.Value))
		{
			if (location.objects.TryGetValue(adjacentTileLocation, out var value5) && value5 is Fence fence && fence.isGate.Value)
			{
	...
	public virtual void toggleGate(bool open, bool is_toggling_counterpart = false, Farmer who = null)
	{
		...
		case 100:
		{
			Vector2 key2 = tileLocation.Value + new Vector2(-1f, 0f);
			if (location.objects.TryGetValue(key2, out var value2) && value2 is Fence fence2 && fence2.isGate.Value && fence2.getDrawSum() == 10)
			{
				fence2.toggleGate(gatePosition.Value != 0, is_toggling_counterpart: true, who);
			}
			break;
		}
		case 10:
		{
			Vector2 key4 = tileLocation.Value + new Vector2(1f, 0f);
			if (location.objects.TryGetValue(key4, out var value4) && value4 is Fence fence4 && fence4.isGate.Value && fence4.getDrawSum() == 100)
			{
				fence4.toggleGate(gatePosition.Value != 0, is_toggling_counterpart: true, who);
			}
			break;
		}
		case 1000:
		{
			Vector2 key3 = tileLocation.Value + new Vector2(0f, 1f);
			if (location.objects.TryGetValue(key3, out var value3) && value3 is Fence fence3 && fence3.isGate.Value && fence3.getDrawSum() == 500)
			{
				fence3.toggleGate(gatePosition.Value != 0, is_toggling_counterpart: true, who);
			}
			break;
		}
		case 500:
		{
			Vector2 key = tileLocation.Value + new Vector2(0f, -1f);
			if (location.objects.TryGetValue(key, out var value) && value is Fence fence && fence.isGate.Value && fence.getDrawSum() == 1000)
			{
				fence.toggleGate(gatePosition.Value != 0, is_toggling_counterpart: true, who);
			}
			break;
		}
		}
	...
	public override bool performToolAction(Tool t)
	{
		...
		else if (!isGate.Value && IsValidRemovalTool(t))
		{
			...
			int debrisType = data?.RemovalDebrisType ?? 14;
			playNearbySoundAll(audioName);
			location.objects.Remove(tileLocation.Value);
			for (int i = 0; i < 4; i++)
			{
			...
			Game1.createRadialDebris(location, debrisType, (int)tileLocation.X, (int)tileLocation.Y, 6, resource: false);
	...
	public virtual bool IsValidRemovalTool(Tool tool)
	{
		...
		FenceData data = GetData();
		List<string> list = data?.RemovalToolIds;
		List<string> list2 = data?.RemovalToolTypes;
		bool result = true;
	...
	public override bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
	{
		...
		if (dropInItem.HasTypeObject() && dropInItem.ItemId == "325")
		{
			...
			if (!isGate.Value)
			{
				...
				if (drawSum == 1500 || drawSum == 110 || drawSum == 1000 || drawSum == 10 || drawSum == 100 || drawSum == 500)
				{
					...
					{
					case 10:
						vector = tileLocation.Value + new Vector2(1f, 0f);
						if (location.objects.GetValueOrDefault(vector) is Fence fence2 && fence2.isGate.Value)
						{
							int drawSum3 = fence2.getDrawSum();
					...
					case 100:
						vector = tileLocation.Value + new Vector2(-1f, 0f);
						if (location.objects.GetValueOrDefault(vector) is Fence fence4 && fence4.isGate.Value)
						{
							int drawSum5 = fence4.getDrawSum();
							if (drawSum5 != 10 && drawSum5 != 110)
							{
								return false;
							}
						}
						break;
					case 1000:
						vector = tileLocation.Value + new Vector2(0f, 1f);
						if (location.objects.GetValueOrDefault(vector) is Fence fence3 && fence3.isGate.Value)
						{
							int drawSum4 = fence3.getDrawSum();
							if (drawSum4 != 500 && drawSum4 != 1500)
							{
								return false;
							}
						}
					...
					case 500:
						vector = tileLocation.Value + new Vector2(0f, -1f);
						if (location.objects.GetValueOrDefault(vector) is Fence fence && fence.isGate.Value)
						{
							int drawSum2 = fence.getDrawSum();
					...
					{
						if (!(vector2 == vector) && location.objects.TryGetValue(vector2, out var value) && value is Fence fence5 && fence5.isGate.Value && fence5.Type == base.Type)
						{
							return false;
						}
					}
					if (heldObject.Value != null)
					{
						...
						heldObject.Value.performRemoveAction();
						heldObject.Value = null;
						Game1.createItemDebris(value2.getOne(), TileLocation * 64f, -1);
					}
					isGate.Value = true;
					if (TryGetData("325", out var data))
					{
		...
		else if (dropInItem.QualifiedItemId == "(O)93" && heldObject.Value == null && !isGate.Value)
		{
			if (!probe)
			{
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 location, float scale, float transparency, float layerDepth, StackDrawType drawStackNumber, Color color, bool drawShadow)
	{
		...
		int drawSum = getDrawSum();
		int tilePosition = fenceDrawGuide[drawSum];
		if (isGate.Value)
		{
			switch (drawSum)
			{
		...
		spriteBatch.Draw(fenceTexture.Value, location + new Vector2(32f, 32f) * scale, Game1.getArbitrarySourceRect(fenceTexture.Value, 64, 128, tilePosition), color * transparency, 0f, new Vector2(32f, 32f) * scale, scale, SpriteEffects.None, layerDepth);
	}

	public bool countsForDrawing(string otherItemId)
	{
		if ((health.Value > 1f || repairQueued.Value) && !isGate.Value)
		{
	...
	public override void draw(SpriteBatch b, int x, int y, float alpha = 1f)
	{
		...
		if (data == null)
		{
			IItemDataDefinition itemDataDefinition = ItemRegistry.RequireTypeDefinition(TypeDefinitionId);
			b.Draw(itemDataDefinition.GetErrorTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocation.X * 64f, tileLocation.Y * 64f)), itemDataDefinition.GetErrorSourceRect(), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-09f);
			return;
		}
		if (health.Value > 1f || repairQueued.Value)
		{
			...
			if (isGate.Value)
			{
				...
				{
				case 10:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, vector + new Vector2(x * 64 - 16, y * 64 - 128)), new Rectangle((gatePosition.Value == 88) ? 24 : 0, 192, 24, 48), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 32 + 1) / 10000f);
					return;
				case 100:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, vector + new Vector2(x * 64 - 16, y * 64 - 128)), new Rectangle((gatePosition.Value == 88) ? 24 : 0, 240, 24, 48), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 32 + 1) / 10000f);
					return;
				case 1000:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, vector + new Vector2(x * 64 + 20, y * 64 - 64 - 20)), new Rectangle((gatePosition.Value == 88) ? 24 : 0, 288, 24, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 - 32 + 2) / 10000f);
					return;
				case 500:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, vector + new Vector2(x * 64 + 20, y * 64 - 64 - 20)), new Rectangle((gatePosition.Value == 88) ? 24 : 0, 320, 24, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 96 - 1) / 10000f);
					return;
				case 110:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, vector + new Vector2(x * 64 - 16, y * 64 - 64)), new Rectangle((gatePosition.Value == 88) ? 24 : 0, 128, 24, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 32 + 1) / 10000f);
					return;
				case 1500:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, vector + new Vector2(x * 64 + 20, y * 64 - 64 - 20)), new Rectangle((gatePosition.Value == 88) ? 16 : 0, 160, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 - 32 + 2) / 10000f);
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, vector + new Vector2(x * 64 + 20, y * 64 - 64 + 44)), new Rectangle((gatePosition.Value == 88) ? 16 : 0, 176, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 96 - 1) / 10000f);
					return;
				}
				num = 17;
			}
			else if (heldObject.Value != null)
			{
				...
				zero += data.HeldObjectDrawOffset;
				switch (drawSum)
				{
				case 10:
					zero.X = data.RightEndHeldObjectDrawX;
					break;
				case 100:
					zero.X = data.LeftEndHeldObjectDrawX;
					break;
				}
				zero *= 4f;
				heldObject.Value.draw(b, x * 64 + (int)zero.X, y * 64 + (int)zero.Y, (float)(y * 64 + 64) / 10000f, 1f);
			}
		}
