...
public class AnimatedSprite : INetObject<NetFields>
{
	...
	{
		string text = overrideTextureName ?? textureName.Value;
		if (!(loadedTexture == text))
		{
			spriteTexture = ((text != null) ? contentManager.Load<Texture2D>(text) : null);
			loadedTexture = text;
	...
	public virtual void UpdateSourceRect()
	{
		...
		{
			int num = SpriteWidth;
			int num2 = SpriteHeight;
			int num3 = textureWidth;
			int num4 = textureHeight;
			SourceRect = GetSourceRect(num3, num, num2, currentFrame);
			if (Texture != null && (SourceRect.Right > num3 || SourceRect.Bottom > num4))
			{
				currentFrame = 0;
				SourceRect = GetSourceRect(num3, num, num2, currentFrame);
			}
