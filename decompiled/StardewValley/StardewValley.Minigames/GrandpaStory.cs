...
public class GrandpaStory : IMinigame
{
	...
	public bool tick(GameTime time)
	{
		...
		case 1:
			...
			if (num % 10000 > grandpaSpeechTimer % 10000 && grandpaSpeech.Count > 0)
			{
				grandpaSpeech.Dequeue();
			}
			if (num < 25000 && grandpaSpeechTimer > 25000 && grandpaSpeech.Count > 0)
			{
				grandpaSpeech.Dequeue();
			}
			if (num < 17000 && grandpaSpeechTimer >= 17000)
			{
		...
		case 3:
			grandpaSpeechTimer += time.ElapsedGameTime.Milliseconds;
			if (grandpaSpeechTimer > 2600 && num <= 2600)
			{
		...
		case 5:
			...
			grandpaSpeechTimer += time.ElapsedGameTime.Milliseconds;
			if (num < 2000 && grandpaSpeechTimer >= 2000)
			{
				Game1.player.currentEyes = 4;
			}
			if (num < 3000 && grandpaSpeechTimer >= 3000)
			{
				Game1.player.currentEyes = 1;
				Game1.player.jitterStrength = 1f;
			}
			if (num < 3500 && grandpaSpeechTimer >= 3500)
			{
				Game1.player.stopJittering();
			}
			if (num < 4000 && grandpaSpeechTimer >= 4000)
			{
				Game1.player.currentEyes = 1;
				Game1.player.jitterStrength = 1f;
			}
			if (num < 4500 && grandpaSpeechTimer >= 4500)
			{
				Game1.player.stopJittering();
				Game1.player.doEmote(28);
			}
			if (num < 7000 && grandpaSpeechTimer >= 7000)
			{
				Game1.player.currentEyes = 4;
			}
			if (num < 8000 && grandpaSpeechTimer >= 8000)
			{
		...
		case 6:
			...
			if (num < 5000 && grandpaSpeechTimer >= 5000)
			{
				Game1.playSound("doorCreak");
			}
			if (num < 6000 && grandpaSpeechTimer >= 6000)
			{
				mouseActive = true;
				Point center = clickableGrandpaLetterRect().Center;
				Game1.setMousePositionRaw((int)((float)center.X * Game1.options.zoomLevel), (int)((float)center.Y * Game1.options.zoomLevel));
			}
			if (clickedLetter)
			{
	...
	public void draw(SpriteBatch b)
	{
		...
		if (drawGrandpa)
		{
			...
			if (grandpaSpeech.Count > 0 && grandpaSpeechTimer > 3000)
			{
				...
				vector *= num;
				float num2 = 3f * num;
				Vector2 position = new Vector2((float)(Game1.viewport.Width / 2) - vector.X / 2f, (float)((int)Utility.getTopLeftPositionForCenteringOnScreen(Game1.viewport, 1294, 730).Y + 669) + 3f);
				position.X -= num2;
				b.DrawString(Game1.dialogueFont, text, position, Color.White * 0.25f, 0f, Vector2.Zero, num, SpriteEffects.None, 1f);
				position.X += num2;
				b.DrawString(Game1.dialogueFont, text, position, Color.White, 0f, Vector2.Zero, num, SpriteEffects.None, 1f);
			}
			if (letterReceived)
			{
		...
		{
			float num3 = 1f - ((float)grandpaSpeechTimer - 7000f) / 2000f;
			b.Draw(texture, Utility.getTopLeftPositionForCenteringOnScreen(Game1.viewport, 1294, 730), new Rectangle(0, 0, 427, 240), Color.White * num3, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
			b.Draw(texture, Utility.getTopLeftPositionForCenteringOnScreen(Game1.viewport, 1294, 730) + new Vector2(22f, 211f) * 3f, new Rectangle(264 + totalMilliseconds % 500 / 250 * 19, 581, 19, 17), Color.White * num3, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
			b.Draw(texture, Utility.getTopLeftPositionForCenteringOnScreen(Game1.viewport, 1294, 730) + new Vector2(332f, 215f) * 3f, new Rectangle(305 + totalMilliseconds % 600 / 200 * 12, 581, 12, 12), Color.White * num3, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
			b.Draw(texture, Utility.getTopLeftPositionForCenteringOnScreen(Game1.viewport, 1294, 730) + new Vector2(414f, 211f) * 3f, new Rectangle(460 + totalMilliseconds % 400 / 200 * 13, 581, 13, 17), Color.White * num3, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
			b.Draw(texture, Utility.getTopLeftPositionForCenteringOnScreen(Game1.viewport, 1294, 730) + new Vector2(189f, 81f) * 3f, new Rectangle(426 + totalMilliseconds % 800 / 400 * 16, 581, 16, 16), Color.White * num3, 0f, Vector2.Zero, 3f, SpriteEffects.None, 1f);
		}
