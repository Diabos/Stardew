...
public class FishingGame : IMinigame
{
	...
	public FishingGame()
	{
		...
		Game1.player.TemporaryItem = tool;
	...
	public void receiveKeyPress(Keys k)
	{
		if (!gameDone)
		{
			...
			if (k == Keys.Escape)
			{
				...
				else if (Game1.activeClickableMenu is BobberBar bobberBar)
				{
	...
	public void draw(SpriteBatch b)
	{
		if (showResultsTimer < 0)
		{
			...
			location.Map.RequireLayer("Back").Draw(Game1.mapDisplayDevice, Game1.viewport, Location.Origin, wrapAround: false, 4);
			...
			location.Map.RequireLayer("Buildings").Draw(Game1.mapDisplayDevice, Game1.viewport, Location.Origin, wrapAround: false, 4);
			location.draw(b);
			b.End();
			...
			b.End();
			b.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null);
			location.Map.RequireLayer("Front").Draw(Game1.mapDisplayDevice, Game1.viewport, Location.Origin, wrapAround: false, 4);
			if (Game1.activeClickableMenu != null)
			{
		...
		{
			Game1.drawWithBorder(Game1.content.LoadString("Strings\\StringsFromCSFiles:FishingGame.cs.10444", score), Game1.textColor, (showResultsTimer <= 7000 && perfectionBonus > 0) ? Color.Lime : Color.White, vector);
		}
		if (showResultsTimer <= 9000)
		{
			vector.Y += 48f;
			Game1.drawWithBorder(Game1.content.LoadString("Strings\\StringsFromCSFiles:FishingGame.cs.12010", fishCaught), Game1.textColor, Color.White, vector);
		}
		if (showResultsTimer <= 7000)
		{
			...
			{
				Game1.drawWithBorder(Game1.content.LoadString("Strings\\StringsFromCSFiles:FishingGame.cs.12011", perfectionBonus), Game1.textColor, Color.Yellow, vector);
			}
			else
			{
				Game1.drawWithBorder(Game1.content.LoadString("Strings\\StringsFromCSFiles:FishingGame.cs.12012"), Game1.textColor, Color.Red, vector);
			}
		}
