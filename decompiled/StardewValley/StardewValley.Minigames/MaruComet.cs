...
public class MaruComet : IMinigame
{
	...
	public void changeScreenSize()
	{
		...
		centerOfScreen = num * new Vector2(Game1.game1.localMultiplayerWindow.Width / 2, Game1.game1.localMultiplayerWindow.Height / 2);
		centerOfScreen.X = (int)centerOfScreen.X;
		centerOfScreen.Y = (int)centerOfScreen.Y;
		cometColorOrigin = centerOfScreen + num * new Vector2(-71 * zoom, 71 * zoom);
	}
	...
	public bool tick(GameTime time)
	{
		...
		if (flybyTimer <= 0)
		{
			...
			flybys.Add(new Vector2(flag ? Game1.random.Next(143 * zoom) : (-8 * zoom), flag ? (8 * zoom) : (-Game1.random.Next(143 * zoom))));
			flybysClose.Add(new Vector2(flag ? Game1.random.Next(143 * zoom) : (-8 * zoom), flag ? (8 * zoom) : (-Game1.random.Next(143 * zoom))));
			flybysFar.Add(new Vector2(flag ? Game1.random.Next(143 * zoom) : (-8 * zoom), flag ? (8 * zoom) : (-Game1.random.Next(143 * zoom))));
		}
		for (int num = flybys.Count - 1; num >= 0; num--)
		{
			flybys[num] = new Vector2(flybys[num].X + 0.8f * (float)time.ElapsedGameTime.Milliseconds, flybys[num].Y - 0.8f * (float)time.ElapsedGameTime.Milliseconds);
			if (cometColorOrigin.Y + flybys[num].Y < centerOfScreen.Y - (float)(143 * zoom / 2))
			{
				flybys.RemoveAt(num);
			}
		}
		for (int num2 = flybysClose.Count - 1; num2 >= 0; num2--)
		{
			flybysClose[num2] = new Vector2(flybysClose[num2].X + 0.8f * (float)time.ElapsedGameTime.Milliseconds * 1.5f, flybysClose[num2].Y - 0.8f * (float)time.ElapsedGameTime.Milliseconds * 1.5f);
			if (cometColorOrigin.Y + flybysClose[num2].Y < centerOfScreen.Y - (float)(143 * zoom / 2))
			{
				flybysClose.RemoveAt(num2);
			}
		}
		for (int num3 = flybysFar.Count - 1; num3 >= 0; num3--)
		{
			flybysFar[num3] = new Vector2(flybysFar[num3].X + 0.8f * (float)time.ElapsedGameTime.Milliseconds * 0.5f, flybysFar[num3].Y - 0.8f * (float)time.ElapsedGameTime.Milliseconds * 0.5f);
			if (cometColorOrigin.Y + flybysFar[num3].Y < centerOfScreen.Y - (float)(143 * zoom / 2))
			{
				flybysFar.RemoveAt(num3);
			}
		}
	...
	public void draw(SpriteBatch b)
	{
		...
		foreach (Vector2 flyby in flybys)
		{
			b.Draw(cometTexture, cometColorOrigin + flyby, new Rectangle(0, 0, 8, 8), Color.White * 0.4f, 0f, Vector2.Zero, zoom, SpriteEffects.None, 0.24f);
		}
		foreach (Vector2 item in flybysClose)
		{
			b.Draw(cometTexture, cometColorOrigin + item, new Rectangle(0, 0, 8, 8), Color.White * 0.4f, 0f, Vector2.Zero, zoom + 1, SpriteEffects.None, 0.24f);
		}
		foreach (Vector2 item2 in flybysFar)
		{
			b.Draw(cometTexture, cometColorOrigin + item2, new Rectangle(0, 0, 8, 8), Color.White * 0.4f, 0f, Vector2.Zero, zoom - 1, SpriteEffects.None, 0.24f);
		}
		...
		float num = SpriteText.getHeightOfString(currentString, Game1.game1.localMultiplayerWindow.Width);
		float num2 = (int)centerOfScreen.Y + 79 * zoom;
		if (num2 + num > (float)Game1.viewport.Height)
		{
			num2 += (float)Game1.viewport.Height - (num2 + num);
		}
		SpriteText.drawStringHorizontallyCenteredAt(b, currentString, (int)centerOfScreen.X, (int)num2, currentStringCharacter, -1, 99999, 1f, 0.99f, junimoText: false, SpriteText.color_Purple, Game1.game1.localMultiplayerWindow.Width);
